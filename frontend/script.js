document.addEventListener("DOMContentLoaded", () => {
            ...
            while (categoryProducts.length < minPerCategory) {
                Object.entries(arch.subCategories).forEach(([subCategory, items]) => {
                    items.forEach((itemName) => {
                        ...
                        const synthetic = {
                            ...base,
                            id: `${categoryId}-auto-${categoryProducts.length + 1}`,
    ...
    let cart = [];
    let wishlist = [];
    window.PRODUCTS = ensureMinCategoryProducts(PRODUCTS);
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data && data.length > 0) {
                    window.PRODUCTS = ensureMinCategoryProducts(data);
                }
            }
        } catch (error) {
    ...
    window.showCategoryView = (categoryId, categoryLabel) => {
        ...
        if (subCategoryList) {
            const dynamicSubCategories = [...new Set(filtered.map(p => p.subCategory).filter(Boolean))];
            subCategoryList.innerHTML = dynamicSubCategories.length
                ? dynamicSubCategories.map(sc => `<li><a href="#" onclick="return false;">${sc}</a></li>`).join('')
                : '<li><a href="#" onclick="return false;">No sub-categories</a></li>';
        }
